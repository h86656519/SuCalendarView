...
public abstract class BaseMonthView extends BaseView {
    ...
    public BaseMonthView(Context context) {
        ...
        currentCalendar.setDay(1);
        currentCalendar.setMonth(1);
        currentCalendar.setYear(2018);

        selectedCalendar.setDay(1);
        selectedCalendar.setMonth(1);
        selectedCalendar.setYear(2018);
    }
   ...
   // final void initMonthWithDate(int year, int month) {
    public void initMonthWithDate(int year, int month) {
        mYear = year;
        mMonth = month;
        initCalendar();
...
//        } else {
        mHeight = CalendarUtil.getMonthViewHeight(year, month, mItemHeight, weekStart);
        //}
    ...
    private void initCalendar() {
        ...
        mNextDiff = CalendarUtil.getMonthEndDiff(mYear, mMonth, 1);

        int preDiff = CalendarUtil.getMonthViewStartDiff(mYear, mMonth, weekStart);
        int monthDayCount = CalendarUtil.getMonthDaysCount(mYear, mMonth);

        mItems = CalendarUtil.initCalendarForMonthView(mYear, mMonth, currentCalendar, weekStart);

        if (mItems.contains(currentDay)) {
            mCurrentItem = mItems.indexOf(currentDay);
        } else {
            mCurrentItem = mItems.indexOf(selectedCalendar);
        }
...
//            mHeight = mItemHeight * mLineCount;
//        } else {
        mHeight = CalendarUtil.getMonthViewHeight(mYear, mMonth, mItemHeight, weekStart);
        //     }
        invalidate();
    }
...
//        }
    ...
    void updateCurrentDate() {
        ...
        if (mItems.contains(currentDay)) {
            ...
            int index = mItems.indexOf(currentDay);
